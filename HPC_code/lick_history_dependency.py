...
analyse the history dependency of 1st lick timing
...
for sessname in sess_list:
    ...
    ax.scatter(filt_prev_trials, filt_delta, color='grey', s=3, alpha=np.arange(tot_trial)/tot_trial)
    ax.plot([xmin-.1, xmax+.1], [intercept+(xmin-.1)*slope, intercept+(xmax+.1)*slope], color='k')
    plt.show()
    
    fig.savefig('Z:\Dinghao\code_dinghao\HPC_all\history_dependency\{}.png'.format(sessname),
                dpi=300,
                bbox_inches='tight')