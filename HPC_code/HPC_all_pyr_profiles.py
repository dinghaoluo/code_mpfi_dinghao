...
def load_good_bad_idx(beh_series):
...
def load_train(npy_filename):
    npy_file = xp.load(npy_filename, allow_pickle=True).item()
...
def get_trial_by_trial_variability(train):
    """
...
def get_trial_matrix(trains, trialtype_idx, max_samples, clu):
    ...
    for idx, trial in enumerate(trialtype_idx):
        ...
        if 0 < trial_length < max_samples:
            temp_matrix[idx, :trial_length] = trains[clu][trial][:]*samp_freq  # *samp_freq to convert to Hz
        elif trial_length > 0:
            temp_matrix[idx, :] = trains[clu][trial][:max_samples]*samp_freq
    return temp_matrix
...
for pathname in paths:
    ...
    good_idx, bad_idx = load_good_bad_idx(beh_df)
    ...
    for i, pyr in tqdm(enumerate(pyr_idx)):
        ...
        baseline_var = get_trial_by_trial_variability(baseline_matrix)
        ctrl_var = get_trial_by_trial_variability(ctrl_matrix)
        stim_var = get_trial_by_trial_variability(stim_matrix)
