...
all_sess_stem = Path(r'Z:\Dinghao\code_dinghao\HPC_nLight_LC_opto\all_sessions')
...
def main(path):
    ...
    detected_onsets  = np.array([f - 1 for f in detected_onsets_raw])  # 3-frame buffer for envelope 
    detected_offsets = np.array([f + 15 for f in detected_offsets_raw])  # same as above
    ...
    if abs(len(detected_onsets) - len(pulse_trains)) > 10:  # if mismatched
        ...
        detected_onsets  = np.array([f - 1 for f in detected_onsets_raw])  # 1-frame buffer for envelope 
        detected_offsets = np.array([f + 15 for f in detected_offsets_raw])  # same as above
        valid_edges      = [True if off - on < min_interval_frames else False 
                            for on, off in zip(detected_onsets, detected_offsets)]
        detected_onsets  = detected_onsets[valid_edges]
        ...
        if abs(len(detected_onsets) - len(pulse_trains)) > 10:  # if still not matching
            ...
            detected_onsets  = np.array([pf[0] - 1 for pf in pulse_frames])
            detected_offsets = np.array([pf[-1] + 15 for pf in pulse_frames])
            valid_edges      = [True if off - on < min_interval_frames else False 
                                for on, off in zip(detected_onsets, detected_offsets)]
            detected_onsets  = detected_onsets[valid_edges]
            detected_offsets = detected_offsets[valid_edges]
