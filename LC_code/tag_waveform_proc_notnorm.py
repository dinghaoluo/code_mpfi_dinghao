...
def spk_w_sem(fspk, clu, nth_clu, spikes_to_load=[]):
    ...
    spks = np.zeros([tot_spks, n_spk_samp])
    for i in range(tot_spks):
        spks[i, :] = max_spks[i, :]  # normalisation
    
    av_spk = spks.mean(0)  # 1d vector for the average tagged spike wf
    
    # sem calculation
    spk_sem = np.zeros(32)
    
    for i in range(32):
        spk_sem[i] = sem(spks[:, i])
...
for pathname in pathLC:    
    ...
    if 1==1:
        ...
        fig.savefig(out_directory + '/' + file_name[42:60] + '_waveforms_notnorm' + '.png')
        
        np.save('Z:\Dinghao\code_dinghao\LC_tagged_by_sess'+file_name[42:60]+'_tagged_spk_notnorm.npy', tagged_spk_dict)
        np.save('Z:\Dinghao\code_dinghao\LC_tagged_by_sess'+file_name[42:60]+'_tagged_sem_notnorm.npy', tagged_sem_dict)
        np.save('Z:\Dinghao\code_dinghao\LC_tagged_by_sess'+file_name[42:60]+'_avg_spk_notnorm.npy', avg_spk_dict)
        np.save('Z:\Dinghao\code_dinghao\LC_tagged_by_sess'+file_name[42:60]+'_avg_sem_notnorm.npy', avg_sem_dict)