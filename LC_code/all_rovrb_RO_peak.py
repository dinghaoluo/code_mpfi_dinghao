...
for clu in list(all_train.items()):
    ...
    #     pass
    if fsa.shape[0]==9201 or cluname not in RO_peaking_keys or len(matched_bouts)<=10:
        pass
    else:
...
ax.set(xlim=(-1,4),
       ylim=(2,6),
       title='trial run onset v run-bout onset (putative Dbh+)',
       xlabel='time (s)',
       ylabel='spike rate (Hz)')
...
fig.savefig('Z:\Dinghao\code_dinghao\LC_all\LC_all_rovrb_avg_putDbh_ROpeaking.png',
            dpi=300,
            bbox_inches='tight')
...
plt.show()
fig.savefig('Z:\Dinghao\code_dinghao\LC_all\LC_all_rovrb_avg_putDbh_ROpeaking_bivariate.png',
            dpi=300,
            bbox_inches='tight')
