...
for clu in list(wfs.items()):
    ...
    if clu_name in tag_list:
        # calculate trough-to-AHP
        ttahp_tagged[clu_name] = (asc_max+wf_min-des_max)*50  # in usec    
        # calculate asymmetry
        ...
        # calculate trough-to-AHP
        ttahp[clu_name] = (asc_max+wf_min-des_max)*50  # in usec    
        # calculate asymmetry
        asym[clu_name] = (b-a)/(b+a)
        # retrieve spike rate 
        sr[clu_name] = spike_rate[clu_num-2]
    
np.save('Z:\Dinghao\code_dinghao\LC_all_tagged\\'+'LC_tagged_ttahp.npy', 
        ttahp_tagged)
